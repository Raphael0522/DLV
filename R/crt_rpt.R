...
#' @param font  Valid values are "Courier", "Arial", "Times", and "fixed". Default is 'fixed'.
#' @param font_size Font size in cm.
...
#'
# @examples
crt_rpt <- function(env_list, output_name, output_type, headers_list, footers_vector, tbl, margin_list = NULL, font = "fixed", font_size = 9){
    ...
    rpt <- reporter::create_report(orientation = "landscape",
                                   ...
                                   output_type = {{output_type}}, font = {{font}}, units = "cm", font_size = {{font_size}}) %>%
      # Assign font size
      ...
      reporter::titles( headers_list$second_line[[1]], headers_list$second_line[[2]], headers_list$second_line[[3]],
                        ...
                        font_size = {{font_size}}, blank_row = "none") %>%
      ...
      reporter::titles(as_vector(headers_list$titles), bold = FALSE, font_size = {{font_size}}, align = "center", header = TRUE, blank_row = "below")
